...
export default async function ConversationsPage() {
  ...
  const gapsCount = conversations?.reduce((sum, conv) => sum + (conv.gaps_identified || 0), 0) || 0;
    ...
    <div style={{ padding: '24px 28px', background: '#F7F4F0', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { label: 'Active', value: activeCount, warn: activeCount > 0 },
          { label: 'Completed', value: completedCount, green: completedCount > 0 },
          { label: 'Gaps identified', value: gapsCount, danger: gapsCount > 0 },
        ].map(k => (
      ...
      <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E2DA', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {conversations && conversations.length > 0 ? (
          conversations.map((conversation) => (
            <div
              key={conversation.id}
              style={{ padding: '14px 16px', borderBottom: '0.5px solid #E8E2DA', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}
            >
              {/* Avatar */}
              <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                  ...
                  <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 11, color: '#9B9088', flexShrink: 0, marginLeft: 8 }}>
                    {conversation.last_message_at ? formatTimeAgo(conversation.last_message_at) : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12, color: '#5F5A57', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '65%' }}>
                    {conversation.last_message || 'No message yet'}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                    {conversation.gaps_identified && conversation.gaps_identified > 0 && (
                      <span style={{ background: '#FEE2E2', color: '#A32D2D', fontSize: 10, fontWeight: 500, padding: '2px 7px', borderRadius: 100 }}>
                        {conversation.gaps_identified} gap{conversation.gaps_identified > 1 ? 's' : ''}
                      </span>
                    )}
                    <span style={{
                      background: conversation.status === 'active' ? '#D1FAE5' : '#F3F0EC',
          ...
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 16, fontWeight: 500, color: '#1A1410', marginBottom: 10 }}>No conversations yet</div>
            <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: '#5F5A57', lineHeight: 1.6, maxWidth: 400, margin: '0 auto' }}>
