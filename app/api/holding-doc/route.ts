...
async function assertHoldingOwnership(holdingId: string, userId: string) {
...
// POST — upload one doc for a holding (inserts a row in holding_documents)
export async function POST(request: NextRequest) {
  try {
    ...
    const filePath = `${userId}/${holdingId}/${Date.now()}-${safeName}`
    ...
    if (uploadError) {
      console.error('[holding-doc POST] storage error:', uploadError)
    ...
    if (dbError || !inserted) {
      console.error('[holding-doc POST] db error:', dbError)
    ...
    console.error('[holding-doc POST] error:', err)
    return NextResponse.json({ error: 'Upload failed' }, { status: 500 })
  }
}

// GET — list all docs for a holding with fresh 1-hour signed URLs
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[holding-doc GET] query error:', error)
      return NextResponse.json({ error: 'Failed to list documents' }, { status: 500 })
    }

    const docs = await Promise.all(
      (rows || []).map(async r => {
        const { data: signed } = await supabase.storage.from(BUCKET).createSignedUrl(r.file_path, 60 * 60)
        return {
          id: r.id,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (storageErr) console.warn('[holding-doc DELETE] storage remove failed (proceeding):', storageErr)
    ...
    console.error('[holding-doc DELETE] error:', err)
